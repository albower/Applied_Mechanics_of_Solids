...
%
filename = '../Input Files/Solution8_34.txt';
...
for lmn=1:nelem    % Loop over all the members
    ...
    a = connect(lmn,1);
    b = connect(lmn,2);
    [kel,rel]=elstif(coord(a,:),coord(b,:),elprops(lmn,:));
    ...
    for i = 1 : 2
        for ii = 1 : ndof
            ...
            Theta(rw) = Theta(rw) + rel(ndof*(i-1)+ii);
            for j = 1 : 2
                for jj = 1 : ndof
...
for i=1:nload   % Loop over joints with external forces
    node=loads(i,1);
    P(ndof*(node-1)+1)=P(ndof*(node-1)+1) + loads(i,2);
    P(ndof*(node-1)+2)=P(ndof*(node-1)+2)+ loads(i,3);
    if (ndof>2)
      P(ndof*(node-1)+3)=P(ndof*(node-1)+3)+ loads(i,4);
    end
end
%
...
title('Thermal deflection of truss')
%
...
annotation(figure1,'textbox',...
    ...
    'String',{'Deformation Scale Factor: 500'},'FontName','Times','FontSize',16);
axis square
axis equal
 end
...
%
function [kel,rel] = elstif(coorda,coordb,props)
        ...
        rel = zeros(length(vec),1);
        if (length(props)==4)  % Thermal expansion
            rel = props(1)*props(2)*props(3)*props(4)/len*vec';
        end
end
