...
%        penaltystif         Penalty stiffness
%
%
% ==================== Read data from the input file ===========================
%
  filename = '../Input Files/Solution8_45.txt';
...
%   Element data will be written to the file specified below
    ...
    [nprops,materialprops,ncoord,ndof,nnode,coords,nelem,maxnodes,connect,nelnodes,...
        elident,nfix,fixnodes,ndload,dloads,nconstraints,constraints,penaltystif] = read_input_file(infile);
    fclose(infile);
    ...
    K = globalstiffness(ncoord,ndof,nnode,coords,nelem,maxnodes,elident,nelnodes,connect,materialprops,dofs,nconstraints,constraints,penaltystif);

    r = globaltraction(ncoord,ndof,nnode,ndload,coords,nelnodes,elident,connect,dloads);
...
%
  ...
  title('Penalty constraint demonstration');
  axis off
...
function [kel,rel] = constraintstif(ndof,coorda,coordb,coordc,dofa,dofb,dofc,penaltystif)
%
%    Constraint to connect node c to element face between nodes a and b
%    (2D only)
%
   xi = norm(coordc-coorda)/norm(coordb-coorda);
   g = (1-xi)*dofa + xi*dofb - dofc;
%  rel is not used in a linear calculation since g=0
%  Included here to allow constraint to be used in a nonlinear calculation....
   rel = penaltystif*[g*(1-xi);g*xi;-g];
   kel = (1-xi)^2*eye(6);
   kel(3,3) = xi^2; kel(4,4) = xi^2;
   kel(5,5) = 1; kel(6,6) = 1;
   kel(1,3) = xi*(1-xi); kel(3,1) = xi*(1-xi);
   kel(1,5) = -(1-xi); kel(5,1) = -(1-xi);
   kel(2,4) = xi*(1-xi); kel(4,2) = xi*(1-xi);
   kel(2,6) = -(1-xi); kel(6,2) = -(1-xi);
   kel(3,5) = -xi; kel(5,3) = -xi;
   kel(4,6) = -xi; kel(6,4) = -xi;
   kel = kel*penaltystif;
end
...
%                            For 2D elements 0 => plane stress; 1 => plane strain
...
function Stif = globalstiffness(ncoord,ndof,nnode,coords,nelem,maxnodes,elident,nelnodes,connect,materialprops,dofs,nconstraints,constraints,penaltystif)
%
...
% Add the constraints to the global stiffness   
   for n = 1:nconstraints
       ...
       [kconst,~] = constraintstif(ndof,coorda,coordb,coordc,dofa,dofb,dofc,penaltystif);
       for m = 1:3
           for i = 1:ndof
...
%==============================Function read_input_file =================
 function [nprops,materialprops,ncoord,ndof,nnode,coords,nelem,maxnodes, ...
                connect,nelnodes,elident,nfix,fixnodes,ndload,dloads,nconstraints,constraints,penaltystif] = read_input_file(infile) 
    ...
    if (nconstraints>0)
        ...
        cellno = cellno + 2;
        penaltystif = str2num(cellarray{1}{cellno});
    end
 end
