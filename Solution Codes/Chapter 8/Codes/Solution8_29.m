function FEM_incompatible_modes
...
%                            For 2D elements elident=0 or 10 means plane stress
%                                            elident=1 or 11 means plane strain
%                            For all elements elident<10 means standard element
%                                             elident>10 means incompatible mode element                                            
...
%
    filename = '../Input Files/solution8_29.txt';
    incompatible_modes = true;  % Set to false to see shear locking with standard elements
...
%
%   u           Nodal displacements.  Let u_i^a be ith displacement component
...
%   Set up stiffness and force
    K = globalmatrices(ncoord,ndof,nnode,coords, ...
...
E = materialprops(1);
 for i = 1:nnode
   ...
   scale = 1.;
   defcoords(3,i) = coords(3,i) - scale*ups1;
   defcoords(2,i) = coords(2,i) + scale*ups2;
   defcoords(1,i) = coords(1,i);
 end
...
set(axes1,'CameraPosition',...
    [-481.220078267144 1.8982306770367 16.4121184549891],'CameraTarget',...
    [-0.000718706149601574 -2.70000004768372 16.6416377274065],...
    'CameraViewAngle',2.16336956781902,'CameraUpVector',[0 1 0],...
    'DataAspectRatio',[1 1 1],...
    'PlotBoxAspectRatio',[1.26789362020887 1 15.0218518357425]);
 axis off
     if (incompatible_modes)
     title({'Incompatible Mode Element Demo', 'Blue - exact solution; red - FEA'})
     else
     title({'Shear Locking Demo', 'Blue - exact solution; red - FEA'})
     end
...
%
   ...
   k_full = zeros(ndof*nelnodes+ndof*ndof);
   ...
   xi0 = zeros(ncoord,1);
   dNdxi = shapefunctionderivs(nelnodes,ncoord,elident,xi0);   
   dxdxi = coord*dNdxi;
   dt0 = det(dxdxi);
...
%       Incompatible modes
        B(1,2*nelnodes+1) = dt0*xi(1)*dxidx(1,1)/dt;
        B(1,2*nelnodes+3) = dt0*xi(2)*dxidx(2,1)/dt;
        B(2,2*nelnodes+2) = dt0*xi(1)*dxidx(1,2)/dt;
        B(2,2*nelnodes+4) = dt0*xi(2)*dxidx(2,2)/dt;
        B(3,2*nelnodes+1) = dt0*xi(1)*dxidx(1,2)/dt;
        B(3,2*nelnodes+3) = dt0*xi(2)*dxidx(2,2)/dt;
        B(3,2*nelnodes+2) = dt0*xi(1)*dxidx(1,1)/dt;
        B(3,2*nelnodes+4) = dt0*xi(2)*dxidx(2,1)/dt;
        if (elident==0 || elident ==10) % Plane stress
           D = E/(1-nu^2)*[1,nu,0;nu,1,0;0,0,(1-nu)/2];  
        elseif (elident==1 || elident==11) % Plane strain
           D = E/(1+nu)/(1-2*nu)*[1-nu,nu,0;nu,1-nu,0;0,0,(1-2*nu)/2];
        end
      else
        B = zeros(6,3*nelnodes+9);
...
%       Incompatible modes
        B(1,3*nelnodes+1) = dt0*xi(1)*dxidx(1,1)/dt;
        B(1,3*nelnodes+4) = dt0*xi(2)*dxidx(2,1)/dt;
        B(1,3*nelnodes+7) = dt0*xi(3)*dxidx(3,1)/dt;
        B(2,3*nelnodes+2) = dt0*xi(1)*dxidx(1,2)/dt;
        B(2,3*nelnodes+5) = dt0*xi(2)*dxidx(2,2)/dt;
        B(2,3*nelnodes+8) = dt0*xi(3)*dxidx(3,2)/dt;
        B(3,3*nelnodes+3) = dt0*xi(1)*dxidx(1,3)/dt;
        B(3,3*nelnodes+6) = dt0*xi(2)*dxidx(2,3)/dt;
        B(3,3*nelnodes+9) = dt0*xi(3)*dxidx(3,3)/dt;
        B(4,3*nelnodes+1) = dt0*xi(1)*dxidx(1,2)/dt;
        B(4,3*nelnodes+4) = dt0*xi(2)*dxidx(2,2)/dt;
        B(4,3*nelnodes+7) = dt0*xi(3)*dxidx(3,2)/dt;
        B(4,3*nelnodes+2) = dt0*xi(1)*dxidx(1,1)/dt;
        B(4,3*nelnodes+5) = dt0*xi(2)*dxidx(2,1)/dt;
        B(4,3*nelnodes+8) = dt0*xi(3)*dxidx(3,1)/dt;
        B(5,3*nelnodes+1) = dt0*xi(1)*dxidx(1,3)/dt;
        B(5,3*nelnodes+4) = dt0*xi(2)*dxidx(2,3)/dt;
        B(5,3*nelnodes+7) = dt0*xi(3)*dxidx(3,3)/dt;
        B(5,3*nelnodes+3) = dt0*xi(1)*dxidx(1,1)/dt;
        B(5,3*nelnodes+6) = dt0*xi(2)*dxidx(2,1)/dt;
        B(5,3*nelnodes+9) = dt0*xi(3)*dxidx(3,1)/dt;
        B(6,3*nelnodes+2) = dt0*xi(1)*dxidx(1,3)/dt;
        B(6,3*nelnodes+5) = dt0*xi(2)*dxidx(2,3)/dt;
        B(6,3*nelnodes+8) = dt0*xi(3)*dxidx(3,3)/dt;
        B(6,3*nelnodes+3) = dt0*xi(1)*dxidx(1,2)/dt;
        B(6,3*nelnodes+6) = dt0*xi(2)*dxidx(2,2)/dt;
        B(6,3*nelnodes+9) = dt0*xi(3)*dxidx(3,2)/dt;
        D = eye(6)*(1-2*nu)/2;
        D(1:3,1:3) = nu;
        D([1,8,15]) = (1-nu);
        D = E/(1+nu)/(1-2*nu)*D;
      ...
      k_full = k_full + (transpose(B)*D*B)*w(intpt)*dt;
   end

   kuu = k_full(1:nelnodes*ndof,1:nelnodes*ndof);
   kua = zeros(nelnodes*ndof,ndof*ndof);
   kau = kua';
   kaa = zeros(ndof*ndof);
   for i = 1 : ndof*ndof
       kaa(i,1:ndof*ndof) = k_full(nelnodes*ndof+i,nelnodes*ndof+1:nelnodes*ndof+ndof*ndof);
       kua(1:nelnodes*ndof,i) = k_full(1:nelnodes*ndof,nelnodes*ndof+i);
       kau(i,1:nelnodes*ndof) = k_full(nelnodes*ndof+i,1:nelnodes*ndof);
   end

   if (elident<10) % Standard element
       kel = kuu;
   else
...
function Stif = globalmatrices(ncoord,ndof,nnode,coords,nelem,maxnodes,elident,nelnodes,connect,materialprops)
%
