...
%
    filename = '../Input Files/Solution8_32.txt';
...
%
    ...
    for cases = 1:2  % Compares solutions with standard and Bbar elements
        w = zeros(nnode*ndof,1);
        if (cases==2)
            elident(:) = elident(:) + 10; % Activate Bbar element
        end
        for step = 1 : nsteps
...
%      Print nodal displacements, element strains && stresses to a file
    ...
    set(axes2,'FontName','Times','FontSize',16,...
     ...
     plot(pforplot(:,1),uforplot(:,1),'LineStyle','--','Color','k','LineWidth',2,'Displayname','Standard element')
     plot(pforplot(:,2),uforplot(:,2),'LineStyle','-','Color','k','LineWidth',2,'Displayname','Bbar element')
