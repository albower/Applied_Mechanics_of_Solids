...
%       integration points for 4 noded plane elemenets to 1
%       (see line 372)
%
...
%
filename = '../Input Files/Solution8_7.txt';
...
%   Fix constrained nodes.
    ...
    u = K\F;
    %
    % Calculate stiffness determinant and check for zero eigenvalues
    %
    detval = det(K);
    disp(['Stiffness Matrix Determinant: ',num2str(detval)])
    [V,D] = eig(K);

    count = 0;
    zeroevallist = [];
    for i = 1:ndof*nnode
        eval = D(i,i)*conj(D(i,i));
        if (eval<1.e-08)
            count = count + 1;
            zeroevallist(count) = i;
        end
    end
    if (length(zeroevallist)>0)
        eigenmode = zeroevallist(3);  % Select the eigenmode here
        u = 0.2*real(V(:,eigenmode));
    end
...
function n = numberofintegrationpoints(ncoord,nelnodes,elident)
   ...
   elseif (ncoord == 2) 
     ...
     if (nelnodes == 4)            %This sets the # integration points to 1
         n = 1;
     end
     if (nelnodes == 8)
         n = 9;
