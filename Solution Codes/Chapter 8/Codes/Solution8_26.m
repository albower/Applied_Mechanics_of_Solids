...
%
%
%       This code solves problem 8.26 from the text
...
%
    filename = '../Input Files/Solution8_26.txt'; 
%
...
%
    ...
    set(axes2,'FontName','Times','FontSize',16,...
        ...
        'Box','On');
    plot(forcevdisp(1,:,1),forcevdisp(2,:,1),'k','Marker','o','LineStyle','none','MarkerSize',8,'DisplayName','FEA');
    exactstress = 0:forcevdisp(2,end,1)/50:forcevdisp(2,end,1);
    for i = 1:length(exactstress)
        if (exactstress(i)>materialprops(3))
            exactstrain(i) = materialprops(4)*((exactstress(i)/materialprops(3))^materialprops(5) - 1);
        else
            exactstrain(i) = 0;
        end
    end
    exactstrain = exactstrain + exactstress/materialprops(1);
    
    plot(exactstrain,exactstress,'k','LineStyle','--','LineWidth',2,'DisplayName','Exact')
    
    xlabel({'Strain $\varepsilon_{33}$'},'FontSize',16, 'Interpreter','latex');
    ylabel({'Stress $\sigma_{33}$'},'FontSize',16, 'Interpreter','latex');
    title('Stress-v-strain for rate independent plastic solid','FontName','Times','FontSize',16)
    
    legend1 = legend(axes2,'show');
    set(legend1,...
        'Position',[0.514064304775767 0.209523809523811 0.321264505338023 0.206190474146893],...
        'Interpreter','latex',...
        'FontSize',14,...
...
%       Bbar correction
      ...
      if (elident>0)  % Plane strain or 3D
         dep = deplas(dtime,stress0(:,:,intpt),eplas0(intpt),dstrain,materialprops);
      elseif (elident==0)   % Plane stress
        [dep,de33] = deplas_planestress(dtime,stress0(:,:,intpt),eplas0(intpt),dstrain,materialprops);
        dstrain(3,3) = de33;
      else
         error('Unrecognized element identifier')
      end
      ...
      % Plane strain or 3D D matrix
      DD =  materialstiffness(elident,dstrain,eplas0(intpt),dep,stress0(:,:,intpt),materialprops);

      D = DD;
      if (elident==0)  % Extract plane stress D matrix from 3D
        D = zeros(3,3);
        D(1,1) = DD(1,1) - DD(3,1)*DD(1,3)/DD(3,3);
        D(1,2) = DD(1,2) - DD(3,2)*DD(1,3)/DD(3,3);
        D(1,3) = DD(1,4) - DD(3,4)*DD(1,3)/DD(3,3);
        D(2,1) = DD(2,1) - DD(3,1)*DD(2,3)/DD(3,3);
        D(2,2) = DD(2,2) - DD(3,2)*DD(2,3)/DD(3,3);
        D(2,3) = DD(2,4) - DD(3,4)*DD(2,3)/DD(3,3);
        D(3,1) = DD(4,1) - DD(3,1)*DD(4,3)/DD(3,3);
        D(3,2) = DD(4,2) - DD(3,2)*DD(4,3)/DD(3,3);
        D(3,3) = DD(4,4) - DD(3,4)*DD(4,3)/DD(3,3);
      end
...
%  Plastic properties - see Applied Mechanics of Solids for notation   
   ...
   e0 = materialprops(4);
   n = materialprops(5);
   rip = true;  % Flag for rate independent material
   if (length(materialprops)>5)  % Rate dependent
...
%
   ...
   err = Y;
   tol = 10^(-06)*Y;
   if (sequiv == 0 || (rip && sequiv<=Y) )
     e = 0.;
   else
     while (err>tol)
       if (rip) % Rate independent
         c = (1+(eplas+e)/e0)^(1/n);
         dfde = -1.5*E/(Y*(1+nu)) - c*( 1/(n*(eplas+e+e0)) );
       else
...
function [e,de33] = deplas_planestress(dt,stress0,eplas,dstrain,materialprops)

% Solve concurrently for plastic strain increment and out-of-plane strain
% increment
...
%  Youngs modulus and Poissons ratio
   ...
   e0 = materialprops(4);
   n = materialprops(5);
   rip = true;  % Rate independent flag
   if (length(materialprops)>5)  % Rate dependent
       edot0 = materialprops(6);
       m = materialprops(7);
       rip = false;
...
%
%  Stress predictors with de33=0
%
   devol = trace(dstrain);
   p0 = trace(stress0);
   
   de = dstrain - eye(3)*devol/3; % Deviatoric strain
   S = stress0 - eye(3)*p0/3 + (E/(1+nu))*de;
   sequiv = sqrt(1.5*sum(S.*S,'all'));
   ...
   tol = 10^(-06)*Y;
   de33 = 0;
   if (sequiv == 0 || (rip && sequive<=Y))%Elastic case
     e = 0.;
     ds11 = (dstrain(1,1) + nu*dstrain(2,2))*E/(1-nu^2);
     ds22 = (nu*dstrain(1,1) + dstrain(2,2))*E/(1-nu^2);
     de33 = -nu*(ds11+ds22)/E; 
   else
     while (err>tol) % Solve concurrently for plastic strain increment and out of plane strain increment
       if (rip) % Rate independent
         c = (1+(eplas+e)/e0)^(1/n);
         dfde = -1.5*E/(Y*(1+nu)) - c*( 1/(n*(eplas+e+e0)) );
       else
       ...
       f = sequiv/Y - 1.5*e*E/(Y*(1+nu))- c;
       dsede33 = 1.5*E/(1+nu)*S(3,3)/(sequiv);  % Derivative of sigma_e^star wrt e33
       dS33de33 = 2*E/(3*(1+nu)); % Derivative of S33star wrt e33
       dfde33 = dsede33/Y;
       beta = 1 - 1.5*E*e/((1+nu)*sequiv);
       s33 = beta*S(3,3) + (p0/3+K*(devol+de33));
       ds33de = - 1.5*E*S(3,3)/((1+nu)*sequiv);
       ds33de33 = 1.5*E*e/((1+nu)*sequiv^2)*dsede33 + beta*dS33de33 + K;
       sol = [e;de33] - [dfde,dfde33;ds33de,ds33de33]\[f;s33];
       de33 = sol(2);
       % Correct the stress predictors for de33
       de = dstrain - eye(3)*(devol+de33)/3; % Deviatoric strain increment
       de(3,3) = de(3,3) + de33;
       S = stress0 - eye(3)*p0/3 + (E/(1+nu))*de;
       sequiv = sqrt(1.5*sum(S.*S,'all'));
       if (sol(1)<0)
...
function D =  materialstiffness(elident,dstrain,eplas0,dep,stress0,materialprops)
%
...
%  Bulk modulus, Youngs modulus and Poissons ratio
   ...
   if (dep<1.e-14)   % Elastic update
      if (elident==1) % Plane strain
        Ivec = [1,1,0];
        D = E/(1+nu)*eye(3);
        D(3,3) = D(3,3)/2;
   ...
   beta = 1. - 1.5*E*dep/((1+nu)*se);
   if (rip) % Rate independent
     gamma = 1.5*E/((1+nu)*se) + beta*( (1/(n*(e0+eplas0+dep)) ) );       
   else
