...
%                            (assumed uniform)
%        nties               No. constraints
%        ties(i,c)           Node1, Node2, Node3 for tie number c
%
% ==================== Read data from the input file ===========================
%
   filename = '../Input Files/Solution8_46.txt';
...
[nprops,materialprops,ncoord,ndof,nnode,coords,nelem,maxnodes,connect,nelnodes,elident,nfix,fixnodes,ndload,dloads,nconstraints,constraints] = read_input_file(infile);
fclose(infile);
...
%   Set up stiffness and force
    ...
    F = zeros(ndof*nnode+ndof*nconstraints,1);
    lmults = zeros(ndof*nconstraints,1);
    K = globalmatrices(ncoord,ndof,nnode,coords, ...
        nelem,maxnodes,elident,nelnodes,connect,materialprops,dofs,nconstraints,constraints,lmults);
...
%   Fix constrained nodes.
    ...
    %
   sol = K\F;
   u = sol(1:ndof*nnode); % Displacements
   lmults = sol(ndof*nnode+1:end);  % Lagrange multipliers
...
%      Print nodal displacements, element strains && stresses to a file
    ...
    title('Lagrange Multiplier constraint demonstration');
...
%================= Tie Stiffness ==================================
%
%    Computes stiffness to tie dof1 of node 1 to dof2 of node 2
%
function [kel,rel] = constraintstif(ndof,coorda,coordb,coordc,dofa,dofb,dofc,lmults)
%  The element force vector is not used in a linear calculation
%  It is included here for possible use in a nonlinear application
   xi = norm(coordc-coorda)/norm(coordb-coorda);
   g = (1-xi)*dofa + xi*dofb - dofc;
   kel = zeros(8,8);
   rel = zeros(8,1);
   rel(1:2) = (1-xi)*lmults(1:2);
   rel(3:4) = xi*lmults(1:2);
   rel(5:6) = -lmults(1:2);
   rel(7:8) = g(1:2);
   kel(1,7) = 1-xi; kel(2,8) = 1-xi;
   kel(3,7) = xi; kel(4,8) = xi;
   kel(5,7) = -1; kel(6,8) = -1;
...
function Stif = globalmatrices(ncoord,ndof,nnode,coords,nelem,maxnodes,elident,nelnodes,connect,materialprops,dofs,nconstraints,constraints,lmults)
%
%   Assemble the global stiffness matrix
%
   Stif = zeros(ndof*(nnode+nconstraints),ndof*(nnode+nconstraints));
...
% Add the constraints to the global stiffness   
   for n = 1:nconstraints
       ...
       coordb = coords(:,b);
       coordc = coords(:,c);
       dofa = dofs(ndof*(a-1)+1:ndof*a);
       dofb = dofs(ndof*(b-1)+1:ndof*b);
       dofc = dofs(ndof*(c-1)+1:ndof*c);
       lmultvals = lmults(ndof*(n-1)+1:ndof*n);
       [kconst,~] = constraintstif(ndof,coorda,coordb,coordc,dofa,dofb,dofc,lmultvals);
       for m = 1:3
           for i = 1:ndof
               rw = ndof*(constraints(m,n)-1) + i;
               for k = 1:3
                   for j = 1:ndof
                       cl = ndof*(constraints(k,n)-1) + j;
                       Stif(rw,cl) = Stif(rw,cl) + kconst(ndof*(m-1)+i,ndof*(k-1)+j);
                   end
               end
               cl = ndof*nnode + ndof*(n-1)+1;
               Stif(rw,cl) = Stif(rw,cl) + kconst(ndof*(m-1)+i,7);
               Stif(cl,rw) = Stif(rw,cl);
               cl = ndof*nnode + ndof*(n-1)+2;
               Stif(rw,cl) = Stif(rw,cl) + kconst(ndof*(m-1)+i,8);
               Stif(cl,rw) = Stif(rw,cl);               
           end
       end
   end
...
%      Print nodal displacements, element strains && stresses:a file
 ...
 function [nprops,materialprops,ncoord,ndof,nnode,coords,nelem,maxnodes, ...
                connect,nelnodes,elident,nfix,fixnodes,ndload,dloads,nconstraints,constraints] = read_input_file(infile) 
...
%
    ...
    cellno = cellno + 2;
    nconstraints = str2num(cellarray{1}{cellno});
    constraints = [];
    if (nconstraints>0)
        cellno = cellno + 3;
        constraints = zeros(3,nconstraints);
        for i = 1 : nconstraints
            for j = 1 : 3
                cellno = cellno + 1;
                constraints(j,i) = str2num(cellarray{1}{cellno});
            end
        end
        cellno = cellno + 2;
    end
 ...
 if (ncoord==2)  % Plot a 2D mesh
     for lmn = 1:nelem
         ...
         if (nelnodes(lmn)==3)
             patch('Vertices',x,'Faces',f2D_3,'FaceColor','none','EdgeColor',color,'LineWidth',2);
         elseif (nelnodes(lmn)==4)
             patch('Vertices',x,'Faces',f2D_4,'FaceColor','none','EdgeColor',color,'LineWidth',2);
         elseif (nelnodes(lmn)==6)
             patch('Vertices',x,'Faces',f2D_6,'FaceColor','none','EdgeColor',color,'LineWidth',2);
         elseif (nelnodes(lmn)==8 || nelnodes(lmn)==9)
             patch('Vertices',x,'Faces',f2D_8,'FaceColor','none','EdgeColor',color,'LineWidth',2);
         end
     end
 elseif (ncoord==3) % Plot a 3D mesh
     for lmn = 1:nelem
         ...
         if (nelnodes(lmn)==4)
             patch('Vertices',x,'Faces',f3D_4,'FaceColor','none','EdgeColor',color,'LineWidth',2);
         elseif (nelnodes(lmn)==10)
             patch('Vertices',x,'Faces',f3D_10,'FaceColor','none','EdgeColor',color,'LineWidth',2);
         elseif (nelnodes(lmn)==8)
             patch('Vertices',x,'Faces',f3D_8,'FaceColor','none','EdgeColor',color,'LineWidth',2);
         elseif (nelnodes(lmn)==20)
             patch('Vertices',x,'Faces',f3D_20,'FaceColor','none','EdgeColor',color,'LineWidth',2);
         end
     end
 end
 axis equal
