function Solution8_23

%
%          Example 2D and 3D hyperelastic FEM code 
%
%
%       This code solves problem 8.23 from the text
...
%       with a nonlinear hyperelastic material.  It will experience
%       serious volumetric locking for most values of material properties
%       of practical interest.   Use the B-bar version of this code
%       in FEM_bbar_hyperelastic.m for more accurate solutions.  
%
%%
...
%
    filename = '../Input Files/Solution8_23.txt';
...
%    
    ...
    set(axes3,'FontName','Times','FontSize',16,...
        ...
        'Box','On')
    exactstretch = 1:0.1:stretchforplot(end);
    plot(stretchforplot,sforplot,'LineStyle','none','Marker','o','MarkerSize',8,'Color','k','DisplayName','FEA')
    if (length(materialprops)==2)
      exactsol = materialprops(1)*(exactstretch.^2 - 1./exactstretch);
    elseif (length(materialprops)==3)
      exactsol = materialprops(1)*(exactstretch.^2 - 1./exactstretch.^4)...
               + materialprops(2)*(exactstretch.^4 - exactstretch.^(-2));
    end
    
    hold on
    plot(exactstretch,exactsol,'LineWidth',2,'LineStyle','-','Color','k','DisplayName','Exact')
    
    
    xlabel({'Stretch $\lambda$'},'Interpreter','latex','FontSize',16);
    ylabel({'Cauchy Stress $\sigma$'},'Interpreter','latex','FontSize',16);
    title('Stress-v-stretch for Mooney-Rivlin solid')
    
    legend1 = legend(axes3,'show');
    set(legend1,...
        'Position',[0.186893114889557 0.690476190476191 0.264892599396158 0.206190474146893],...
        'Interpreter','latex',...
        'FontSize',14,...
...
%
      ...
      D = materialstiffness(ndof,ncoord,B,J,materialprops);
      ...
      kel = kel + (transpose(Abar)*D*G*Abarstar - P.*S)*w(intpt)*dt;
   end
...
function stress = Kirchhoffstress(ndof,ncoord,B,J,materialprops)
   ...
   if (length(materialprops)==2)  % Neo-Hookean material
       mu1 = materialprops(1);
       K1 = materialprops(2);
   elseif (length(materialprops)==3) % Mooney-Rivlin
       mu1 = materialprops(1);
       mu2 = materialprops(2);
       K1 = materialprops(3);       
   end
   ...
   if (length(materialprops)==3)% Mooney-Rivlin
       stress = stress + mu2*trace(B)/J^(5/3)*(B-trace(B)/3*eye(3)) -...
           mu2/J^(5/3)*(B*B - sum(B.*B,'all')*eye(3)/3);
   end
...
function D = materialstiffness(ndof,ncoord,B,J,materialprops)
   ...
   if (ndof==2)   % Plane strain
      ...
      if (length(materialprops)==3)
          Z = B*B;
          zvec = [Z(1,1),Z(2,2),Z(1,2)];
          tauvec = mu2/J^(5/3)*(trace(B)*bvec - trace(B)^2*ivec/3 - zvec + trace(Z)*ivec/3);
          Dhat = eye(3);
          Dhat(3,3) = 0.5;           
          Dhat = Dhat*mu2*trace(B)/J^(5/3);
          Dhat = Dhat - mu2/J^(5/3)*[2*B(1,1),0,B(1,2);0,2*B(2,2),B(1,2);B(1,2),B(1,2),(B(1,1)+B(2,2))/2];
          Dhat = Dhat + mu2/J^(5/3)*(tensorprod(bvec,ivec) - 2*trace(B)/3*tensorprod(ivec,ivec) + 2/3*tensorprod(ivec,bvec));
          Dhat = Dhat - 5/3*tensorprod(tauvec,bivec);
          D = D + Dhat;
      end
   else
      D = eye(6);
      ...
      if (length(materialprops)==3)
          Z = B*B;
          zvec = [Z(1,1),Z(2,2),Z(3,3),Z(1,2),Z(1,3),Z(2,3)];
          tauvec = mu2/J^(5/3)*(trace(B)*bvec - trace(B)^2*ivec/3 - zvec + trace(Z)*ivec/3);
          Dhat = eye(6);
          Dhat(4,4) = 0.5;  
          Dhat(5,5) = 0.5;
          ...
          Dhat = Dhat*mu2*trace(B)/J^(5/3);
           Dhat = Dhat - mu2/J^(5/3)*...
                 [2*B(1,1),0,0,B(1,2),B(1,3),0;...
                  0,2*B(2,2),0,B(1,2),0,B(2,3);...
                  0,0,2*B(3,3),0,B(1,3),B(2,3);...
                  B(1,2),B(1,2),0,(B(1,1)+B(2,2))/2, B(2,3)/2, B(1,3)/2;...
                  B(1,3),0,B(1,3),B(2,3)/2,(B(1,1)+B(3,3))/2, B(1,2)/2;...
                  0,B(2,3),B(2,3),B(1,3)/2,B(1,2)/2,(B(2,2)+B(3,3))/2];
          Dhat = Dhat + mu2/J^(5/3)*(tensorprod(bvec,ivec) - 2*trace(B)/3*tensorprod(ivec,ivec) + 2/3*tensorprod(ivec,bvec));
          Dhat = Dhat - 5/3*tensorprod(tauvec,bivec);
          D = D + Dhat;
...
function sforplot = print_results(outfile, ...
    nprops,materialprops,ncoord,ndof,nnode,coords, ...
    nelem,maxnodes,connect,nelnodes,elident, ...
...
%
    for lmn = 1:nelem
        ...
        for intpt = 1 : npoints
            ...
            stress = Kirchhoffstress(ndof,ncoord,B,J,materialprops)/J;
            sforplot = stress(3,3);
            if (ncoord == 2)
