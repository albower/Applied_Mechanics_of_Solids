...
%       with a nonlinear hypoelastic material.  It will experience
%       serious volumetric locking for most values of material properties
%       of practical interest.   For further details, and a way to fix
%       this issue, see problems/solutions 8.18 and 8.30 from 
...
%
    filename = '../Input Files/Solution8_21.txt';
...
%
%       Save stress-v-strain for plotting (this assumes element width=1)
%
        fvu(2,step) = loadfactor*sum(F);
        fvu(1,step) = w(5)/coords(ndof,end);
    end
...
%      Print nodal displacements, element strains && stresses to a file
    ...
    for i = 1:nnode
        ...
        set(axes3,'FontName','Times','FontSize',16,...
            ...
            'Box','On')
        exactstrain = 0:fvu(1,end)/25:fvu(1,end);
        exactstress = materialprops(1).*(exactstrain./materialprops(2)).^(1/materialprops(3));
        plot(fvu(1,:),fvu(2,:),'LineStyle','none','Marker','o','MarkerSize',8,'Color','k','DisplayName','FEA')
        hold on
        plot(exactstrain,exactstress,'LineWidth',2,'LineStyle','-','Color','k','DisplayName','Exact')
    
    
       xlabel({'Strain $\epsilon$'},'Interpreter','latex','FontSize',16);
       ylabel({'Stress $\sigma$'},'Interpreter','latex','FontSize',16);
       
       legend1 = legend(axes3,'show');
       set(legend1,...
           'Position',[0.44760740060384 0.295238095238097 0.264892599396158 0.206190474146894],...
           'Interpreter','latex',...
           'FontSize',14,...
...
%
   ...
   for intpt = 1 : npoints
      ...
      strainvec = B*uvec';
      [stress,eps33] = materialstress(ndof,ncoord,strainvec,elident,materialprops);
      if (ndof==2) % Plane strain or plane stress
          stressvec = [stress(1,1);stress(2,2);stress(1,2)];
      else  % 3D
          stressvec = [stress(1,1);stress(2,2);stress(3,3);stress(1,2);stress(1,3);stress(2,3)];
      end      
      
%     Note that DD is returned as the full 6x6 matrix for plane stress
      if (elident==0)
          strainvec = [strainvec(1),strainvec(2),eps33,strainvec(3),0,0]; % Need full 3D strain for plane stress
      end
      D3D = materialstiffness(ndof,ncoord,strainvec,elident,materialprops);
      D = D3D;
      if (elident==0)  % Modify 3D matrix for plane stress
        D = zeros(3,3);
...
function [stress,eps33] = materialstress(ndof,ncoord,strainvec,elident,materialprops)
...
%  Bulk modulus
   ...
   dl = eye(3);
   eps33 = -strainvec(1) - strainvec(2);
   if (elident==0) % Plane stress
      strain = [strainvec(1),strainvec(3)/2,0;...
                strainvec(3)/2,strainvec(2),0;...
                0,0,eps33];
      strainvec3D = [strainvec(1),strainvec(2),eps33,strainvec(3),0,0]; % Switch to 3D strain vec
      ...
      while (err>tol)
        ...
        D = materialstiffness(ndof,ncoord,strainvec3D,elident,materialprops);
        eps33 = eps33 - stress(3,3)/D(3,3);
        strain(3,3) = eps33;
        strainvec3D(3) = eps33;
        err = abs(stress(3,3));
      end

   elseif (elident==1)  % Plane strain
      strain = [strainvec(1),strainvec(3)/2,0;...
                strainvec(3)/2,strainvec(2),0;...
                0,0,0];
   else  %3D
...
function D = materialstiffness(ndof,ncoord,strainvec,elident,materialprops)
...
%  Bulk modulus
   ...
   if (elident==1)  % Plane strain
      ...
      if (ee>0)
        ...
        D(1:3,1:3) = D(1:3,1:3)+(K-2*Et/9)*ones(3) + (2*Et/3)*eye(3);
        D(4,4) = D(4,4) + (Et/3);
        D(5,5) = D(5,5) + (Et/3);
        D(6,6) = D(6,6) + (Et/3);
      end
...
%
%  Can comment out function below for recent MATLAB versions
...
%      Print nodal displacements, element strains && stresses:a file
    ...
    for lmn = 1:nelem
        ...
        for intpt = 1 : npoints
            ...
            e = B*uvec';
            stress = materialstress(ndof,ncoord,e,elident(lmn),materialprops);
