function Solution8_31
%
%
%          Example 2D and 3D hypoelastic FEM code 
%
%
%       This code solves problem 8.31 from the text
...
%        elident(i)          elident<10 - full integration
%                            elident>10 - reduced integration
%                            elident>100 - reduced integration with hourglass control 
...
%
    filename = '../Input Files/Solution8_31.txt';
...
%
    ...
    pmag = 40;   % Magnitude of pressure applied to inner bore
    
    pforplot = zeros(nsteps+1,2);
    uforplot = pforplot;
    
    for cases = 1:3  % Compares solutions with full and reduced integration
        w = zeros(nnode*ndof,1);
        if (cases==2)
            elident(:) = elident(:) + 100; % Activate reduced integration with hourglass control
        end
        if (cases==3)
            elident(:) = elident(:)-90;    % Reduced integration with no hourglass control
            pmag = 10;
        end
        for step = 1 : nsteps
            
            loadfactor = pmag*step/nsteps;
            ...
            %
            %       Save pressure-v-displacement for plotting
            %
            if (cases<3)
            pforplot(step+1,cases) = loadfactor;
            uforplot(step+1,cases) = w(2);
            end
        end
    end
...
%      Print nodal displacements, element strains && stresses to a file
    ...
    % Create a plot of the deformed mesh with no hourglass control
    ...
    set(axes2,'FontName','Times','FontSize',16,...
     ...
     title('Predicted deformation without hourglass control')
     %
     % Plot the pressure-v-displacement curve
     %
     figure1 = figure('NumberTitle','off','Name','Figure','Color',[1 1 1]);
     ...
     plot(pforplot(:,1),uforplot(:,1),'LineStyle','--','Color','k','LineWidth',2,'Displayname','Full Integration')
     plot(pforplot(:,2),uforplot(:,2),'LineStyle','-','Color','k','LineWidth',2,'Displayname','Reduced Integration (hourglass control)')
     ylabel({'Displacememt $u_r(a)$'},'Interpreter','latex','FontSize',16);
     xlabel({'Pressure $p$'},'Interpreter','latex','FontSize',16);
     title('Pressure-v-displacement for pressurized cylinder')
     legend1 = legend(axes3,'show');
     set(legend1,...
         'Position',[0.175698326612258 0.740476190476191 0.685015959102028 0.183333333333353],...
         'Interpreter','latex',...
         'FontSize',14,...
...
%
   if (elident<10)
      npoints = numberofintegrationpoints(ncoord,nelnodes,elident);
       ...
       npoints = numberofreducedintegrationpoints(ncoord,nelnodes,elident);
   end
...
%  Hourglass control (quads and bricks only)
   if (elident>100 && (nelnodes==4||nelnodes==8) )
      xi(1:ncoord) = 0;
      ...
      [Gammabi,nvec] = hourglassbasevectors(ncoord,nelnodes);
      gamma = zeros(nelnodes,nvec);
      for i = 1:nvec
          gamma(:,i) = Gammabi(:,i) - dNdx*coord*Gammabi(:,i);
      end
      shearmodulus = (2/3)*materialprops(3)*materialprops(1)/materialprops(2);
      scale = shearmodulus*sum(dNdx.*dNdx,'all');       
        ...
        for i  = 1 : ndof
          for b  = 1 : nelnodes
            for k  = 1 : ndof
              row = ndof*(a-1)+i;
              col = ndof*(b-1)+k;
              kel(col,row) = kel(col,row) + 0.01*scale*gab(a,b)*dt*4;
            end
          end
        end
      end      
...
function [G,nvec] = hourglassbasevectors(ncoord,nelnodes)
 
   if (nelnodes==4&&ncoord==2) % 4 noded quad
     nvec = 1;
     G(1,1) = 1;
     G(2,1) = -1;
     G(3,1) = 1;
     G(4,1) = -1;
   elseif (nelnodes==8&&ncoord==3) % 8 noded brick
     nvec = 4;
     G(:,1) = [1,1,-1,-1,-1,-1,1,1]';
     G(:,2) = [1,-1,-1,1,-1,1,1,-1]';
     G(:,3) = [1,-1,1,-1,1,-1,1,-1]';
