...
%
%
%       This code solves problem 8.27 from the text
...
%
    filename = '../Input Files/Solution8_27.txt'; 
%
...
%
  ...
  ncycles = 3;
  loadfactor = 0;
  forcevdisp = zeros(2,ncycles*nsteps+1);
  count = 1;
  for cycle = 1:ncycles
    ...
    for step = 1 : nsteps
       ...
       if (cycle==1)
        loadfactor = loadfactor + 1/nsteps;
        elseif (cycle==2)
            loadfactor = loadfactor - 2/nsteps;
        elseif (cycle ==3)
            loadfactor = loadfactor + 1/nsteps;
        end
        ...
        while ((err1>tol) && (nit<maxit))          % Newton Raphson loop
            ...
            [R,K,stress1,alpha1] = globalmatrices(dt,ncoord,ndof,nnode,coords, ...
                nelem,maxnodes,elident,nelnodes,connect,materialprops,stress0,alpha0,w);
   ...
   % update state
        ...
        forcevdisp(2,count) = loadfactor*dloads(5,1);
        forcevdisp(1,count) = u(15);
        count = count + 1;   
...
%
        fprintf(outfile,'\n\n Step %f Load %f\n',step,loadfactor);
         ...
         print_results(outfile, ...
            ...
            nelem,maxnodes,connect,nelnodes,elident,stress1,alpha1, ...
...
%
    ...
    set(axes2,'FontName','Times','FontSize',16,...
     ...
     set(axes2,'FontName','Times','FontSize',16,...
         ...
         'Box','On');
     plot(forcevdisp(1,:),forcevdisp(2,:),'k','Marker','o','LineStyle','none','MarkerSize',8,'DisplayName','FEA');
     
     exactstress = [0,15,18,-12,-18,0];
     exactstrain = [0,15/1000,18/1000+(18-15)/10,-12/1000+0.3,-18/1000+(-18+15)/10,(-18+15)/10];
...
%
     ...
     set(legend1,...
         'Position',[0.147992876204338 0.683333333333337 0.321264505338023 0.206190474146893],...
         'Interpreter','latex',...
         'FontSize',14,...
...
function [rel,kel,stress1,alpha1] = elforcnstif(dtime,ncoord,ndof,nelnodes,elident,coord,materialprops,stress0,alpha0,displacement)
%
...
%       Bbar correction
      ...
      [stress,alpha,D] = materialupdate(ndof,stress0(:,:,intpt),alpha0(:,:,intpt),dstrain,materialprops);
...
function [stress1,alpha1,D] = materialupdate(ndof,stress0,alpha0,dstrain,materialprops)
...
%
   ...
   se = sqrt(1.5*sum((S-alpha0).*(S-alpha0),'all'));

   if (se<Y)  % Elastic update 
     stress1 = S + (p0/3+K*devol)*eye(3);
     alpha1 = alpha0;
     ...
     dep = (se-Y)/(gamma+c);
     lambda = gamma*dep/(Y+c*dep);
     dlamddep = gamma*Y/(Y+c*dep)^2;
     stress1 = (S+lambda*alpha0)/(1+lambda) + (p0/3+K*devol)*eye(3);
     alpha1 = (alpha0  + dep*c*(S+lambda*alpha0)/(1+lambda)/Y )/(1+c*dep/Y);    

     if (ndof==2)  % Plane strain
         
         stressvec = [S(1,1),S(2,2),S(1,2)];
         alphavec = [alpha0(1,1),alpha0(2,2),alpha0(1,2)];
         Ivec = [1,1,0];
         D = E/((1+lambda)*(1+nu))*eye(3);
         D(3,3) = D(3,3)/2;
         
     else   % 3D
         stressvec = [S(1,1),S(2,2),S(3,3),S(1,2),S(1,3),S(2,3)];
         alphavec = [alpha0(1,1),alpha0(2,2),alpha0(3,3),alpha0(1,2),alpha0(1,3),alpha0(2,3)];
         Ivec = [1,1,1,0,0,0];
         D = E/((1+lambda)*(1+nu))*eye(6);
         D(4,4) = D(4,4)/2;
         D(5,5) = D(5,5)/2;
         D(6,6) = D(6,6)/2;
     end
     
     D = D + (K-E/(3*(1+lambda)*(1+nu)))*tensorprod(Ivec,Ivec);
     D = D - gamma*dlamddep*tensorprod(stressvec-alphavec,stressvec-alphavec)/...
               ( (1+lambda)^2*se*(c+gamma) );
...
function [resid,Stif,stress1,alpha1] = globalmatrices(dt,ncoord,ndof,nnode,coords, ...
                nelem,maxnodes,elident,nelnodes,connect,materialprops,stress0,alpha0,dofs)
%
...
%
    ...
    n = nelnodes(lmn);
    ident = elident(lmn);
    [rel,kel,stress1(:,:,:,lmn),alpha1(:,:,:,lmn)] = elforcnstif(dt,ncoord,ndof,n,ident,lmncoord,materialprops,stress0(:,:,:,lmn),alpha0(:,:,:,lmn),lmndof);
%
...
function print_results(outfile, ...
    nprops,materialprops,ncoord,ndof,nnode,coords, ...
    nelem,maxnodes,connect,nelnodes,elident,stress,alpha, ...
        ...
        %       Bbar correction
            ...
            if (ncoord == 2)
                ...
                fprintf(outfile,'%5d %7.4f %7.4f %9.4f %9.4f %9.4f %9.4f %9.4f %9.4f \n', ...
                    intpt,x(1),x(2),strain(1),strain(2),strain(3),stress(1,1,intpt,lmn),stress(2,2,intpt,lmn),stress(1,2,intpt,lmn));
