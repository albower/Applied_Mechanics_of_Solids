function Solution8_10
%
%
%          Example 2D and 3D linear elastic FEM code 
%
%       This code solves problem 8.10 from the text
...
%
    filename = '../Input Files/Solution8_10.txt';
...
%   Element data will be written to the file specified below
    ...
    [nprops,materialprops,ncoord,ndof,nnode,coords,nelem,maxnodes,connect,nelnodes,elident,nfix,fixnodes,ndload,dloads,nbforces,bforces] = read_input_file(infile);
    fclose(infile);
...
%
    ...
    if (nbforces>0)
        F = F + globalbforce(ncoord,ndof,nnode,coords,maxnodes,elident,nelnodes,connect,nbforces,bforces);
    end
...
%      Print nodal displacements, element strains && stresses to a file
    [stresspoints,stressforplot] = print_results(outfile, ...
    ...
    scalefactor = 1.0;   % Change this to scale the displacements as needed
    for i = 1:nnode
        for j = 1:ndof
    ...
    %% Plot displacements and stresses and compare with exact solution
    ...
    set(axes3,'FontName','Times','FontSize',16,...
        ...
        'Box','On')
    plot(coords(1,1:6),u(1:2:12),'Marker','o','LineStyle','none','Color','k','LineWidth',2,'Displayname','FEA')
    E = materialprops(1);  nu = materialprops(2);
    %   Exact displacements (bar length is 5, body force is 1)
    xex = 0:0.05:5;
    scale = (1+nu)*(1-2*nu)/(1-nu)/E/2;
    uex = scale*xex.*(10-xex);
    plot(xex,uex,'LineStyle','-','Color','k','LineWidth',2,'Displayname','Exact')
    ...
    set(legend1,...
        'Position',[0.207738097579706 0.773412701534848 0.169642854801246 0.115476187354042]);
    title('1D Bar loaded by body forces')
    %
    figure1 = figure('NumberTitle','off','Name','Figure','Color',[1 1 1]);
    axes3 = axes('Parent',figure1,...
        'Position',[0.175 0.207142857142857 0.73 0.717857142857145]);
    ...
    set(axes3,'FontName','Times','FontSize',16,...
        'LineWidth',2,'XGrid','on','YGrid','on',...
        'Box','On')
    plot(stresspoints,stressforplot,'LineStyle','none','LineWidth',2,'Marker','o','Color','k','Displayname','FEA');
    sex = (5-xex);  % Sorry about the variable name.   exact stresses.
    plot(xex,sex,'LineStyle','-','Color','k','LineWidth',2,'Displayname','Exact')  ;
    xlabel({'Position $x_1$'},'Interpreter','latex','FontSize',16);
    ylabel({'Stress $\sigma_{11}$'},'Interpreter','latex','FontSize',16);
    legend1 = legend(axes3,'show');
    set(legend1,...
        'Position',[0.227380954722563 0.351984130106278 0.169642854801246 0.115476187354042]);
    title('1D Bar loaded by body forces')
end
...
%      elident            Element identifier: 0 for plane stress; 1 for plane strain
...
%      B(i,a)             B matrix defined so strain = B*element_dofs
%      D(i,j)             Matrix of elastic constants
%
%
...
function rel = elbforc(ncoord,ndof,nelnodes,elident,coord,bforce)
%
%  Assemble the residual force vector
%
%    Arguments;
%
%      ncoord             No. coordinates (2 or 3 for 2D or 3D problem)
...
%
   ...
   for intpt = 1 : npoints
      ...
      if (ndof==2)
        ...
        rel(1:2:2*nelnodes-1) = rel(1:2:2*nelnodes-1) + N(1:nelnodes)*bforce(1)*w(intpt)*dt;
        rel(2:2:2*nelnodes) = rel(2:2:2*nelnodes) + N(1:nelnodes)*bforce(2)*w(intpt)*dt;
      else
        rel(1:3:3*nelnodes-2) = rel(1:3:3*nelnodes-2) + N(1:nelnodes)*bforce(1)*w(intpt)*dt;
        rel(2:3:3*nelnodes-1) = rel(2:3:3*nelnodes-1) + N(1:nelnodes)*bforce(2)*w(intpt)*dt;    
        rel(3:3:3*nelnodes-1) = rel(3:3:3*nelnodes) + N(1:nelnodes)*bforce(3)*w(intpt)*dt; 
      end
   end
...
function r = globalbforce(ncoord,ndof,nnodes,coords,maxnodes,elident,nelnodes,connect,nbforce,bforces)
%
%   Assemble the global stiffness matrix
%
   r = zeros(ndof*nnodes,1);
...
%
    ...
    n = nelnodes(lmn);
    ident = elident(lmn);
    rel = elbforc(ncoord,ndof,n,ident,lmncoord,bforces(2:end,nf));
%
...
function [stresspoints,stressforplot] =  print_results(outfile, ...
    nprops,materialprops,ncoord,ndof,nnode,coords, ...
    nelem,maxnodes,connect,nelnodes,elident, ...
...
%      Print nodal displacements, element strains && stresses:a file
    ...
    % Arrays storing stresses for plotting
    npoints = numberofintegrationpoints(ncoord,nelnodes(1));
    stressforplot = zeros(npoints*nelem,1);
    stresspoints = stressforplot;
    count = 0;
    ...
    for lmn = 1:nelem
        ...
        for intpt = 1 : npoints
            ...
            % Store s11 and position for plotting
            count = count + 1;
            stressforplot(count) = s(1);
            stresspoints(count) = x(1);
        end
    end
end
 ...
 function [nprops,materialprops,ncoord,ndof,nnode,coords,nelem,maxnodes, ...
                connect,nelnodes,elident,nfix,fixnodes,ndload,dloads,nbforces,bforces] = read_input_file(infile) 
...
%
 ...
 %
%    No. elements with body forces, with the body forces
%
    cellno = cellno + 2;
    nbforces = 0;
    bforces = [];
    if (cellno<numcells)
        nbforces = str2double(cellarray{1}{cellno});
        cellno = cellno + 2;
        bforces = zeros(1+ndof,nbforces);
        for i = 1 : nbforces
           cellno = cellno + 1;
           bforces(1,i) = str2double(cellarray{1}{cellno});
           for j = 1 : ndof
             cellno = cellno + 1;
             bforces(j+1,i) = str2double(cellarray{1}{cellno});
           end
        end
    end   
 end
