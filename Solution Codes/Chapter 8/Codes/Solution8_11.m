function Solution8_11
%
%
%          Example 2D and 3D linear elastic FEM code 
%
%
%       This code solves problem 8.11 from the text
...
%
%filename = '';
filename = '../Input Files/Solution8_11.txt';
...
%      Print nodal displacements, element strains && stresses to a file
    [stresspoints,stressvals] = print_results(outfile, ...
...
%   Compare the FEA predictions to the exact solution
%
%   Parameters for pressurized cylinder
    a = 1;
    b = 3;
    p = 1.;
    E = 10;
    ...
    set(axes3,'FontName','Times','FontSize',16,...
        ...
        'Box','On')
    plot(stresspoints,stressvals(1,:),'Marker','o','LineStyle','none','Color','k','LineWidth',2,'Displayname','\sigma_{rr} (FEA)')
    plot(stresspoints,stressvals(2,:),'Marker','o','MarkerFaceColor','k','LineStyle','none','Color','k','LineWidth',2,'Displayname','\sigma_{\theta\theta} (FEA)')
    
    exacts = p*a^2/(b^2-a^2)*(1-b^2./exactx.^2);
    plot(exactx,exacts,'LineStyle','--','Color','k','LineWidth',2,'Displayname','\sigma_{rr} Exact')
    exacts = p*a^2/(b^2-a^2)*(1 + b^2./exactx.^2);
    plot(exactx,exacts,'LineStyle','-','Color','k','LineWidth',2,'Displayname','\sigma_{\theta\theta} Exact')
    xlabel({'Position $r$'},'Interpreter','latex','FontSize',16);
    ylabel({'Stress'},'Interpreter','latex','FontSize',16);
    title('Stresses in a pressurized cylinder')
    legend1 = legend(axes3,'show');
    set(legend1,...
        'Position',[0.658630964913887 0.612301594468344 0.230654749371825 0.298809515294575],...
        'Interpreter','tex',...
        'FontSize',14,...
    ...
    %
    xex = exactx;
    uex = (1+nu)*a^2*b^2*p/(b^2-a^2)/E.*(1./xex + (1-2*nu).*xex./b^2);
    figure1 = figure('NumberTitle','off','Name','Figure','Color',[1 1 1]);
    axes3 = axes('Parent',figure1,...
        'Position',[0.175 0.207142857142857 0.73 0.717857142857145]);
    hold(axes3,'on');
    set(axes3,'FontName','Times','FontSize',16,...
        'LineWidth',2,'XGrid','on','YGrid','on',...
        'Box','On')
    plot(coords(1,1:21),u(1:2:41),'Marker','o','LineStyle','none','Color','k','LineWidth',2,'Displayname','FEA')
    plot(xex,uex,'LineStyle','-','Color','k','LineWidth',2,'Displayname','Exact')
    xlabel({'Position $r$'},'Interpreter','latex','FontSize',16);
    ylabel({'Displacement $u_r$'},'Interpreter','latex','FontSize',16);
    title('Displacement in a pressurized cylinder')
    legend1 = legend(axes3,'show');
    set(legend1,...
        'Position',[0.640773822056743 0.671031751850306 0.230654749371825 0.219444438625891],...
        'Interpreter','latex',...
        'FontSize',14,...
        'FontName','Times New Roman');
...
%
   ...
   for intpt = 1 : npoints
      ...
      if (ndof==2)  % 2D
          if (elident<2)  % Plane strain or plane stress
          ...
          elseif (elident==2)   % Axisymmetric
              ...
              B(1,1:2:2*nelnodes-1) = dNdx(1:nelnodes,1);
              B(2,2:2:2*nelnodes) = dNdx(1:nelnodes,2);
              B(3,1:2:2*nelnodes-1) = N(1:nelnodes)/x(1);
              B(4,1:2:2*nelnodes-1) = dNdx(1:nelnodes,2);
              B(4,2:2:2*nelnodes) = dNdx(1:nelnodes,1);
              D = E/(1+nu)/(1-2*nu)*[1-nu,nu,nu,0;...
                  nu,1-nu,nu,0;...
                  nu,nu,1-nu,0;...
                  0,0,0,(1-2*nu)/2];
          else
              disp(' Unrecognized element identifier ')
              disp(' Use 0 for plane stress, 1 for plane strain, 2 for axisymmetric ')
              error('Stopping');
          end
      else  %3D
...
function r = eldload(ncoord,ndof,nfacenodes,elident,coords,traction)
    ...
    for intpt = 1:npoints
        ...
        if (ncoord == 2)
            ...
            if (elident==2)   % Axisymmetric 2D element
                x = coords*N;
                dt = dt*2*pi*x(1);
            end
        elseif (ncoord == 3)
...
function [stresspoints,stressvals] = print_results(outfile, ...
    nprops,materialprops,ncoord,ndof,nnode,coords, ...
    nelem,maxnodes,connect,nelnodes,elident, ...
...
%      Print nodal displacements, element strains && stresses:a file
    ...
    stressvals = zeros(2,npoints*nelem);
    stresspoints = zeros(npoints*nelem,1);
    count = 0;
    ...
    for lmn = 1:nelem
        ...
        fprintf(outfile,' \n Element; %d ',lmn);
        if (ncoord==2)
             if (elident(lmn)<2) % Plane strain or stress
                fprintf(outfile,'  \n int pt    Coords          e_11      e_22     e_12      s_11       s_22      s_12 \n');
             else  % Axisymmetric
                fprintf(outfile,'  \n int pt    Coords          e_rr      e_zz     e_qq      e_rq      s_rr       s_zz      s_qq      s_rq \n');
            end
        elseif (ncoord == 3)
        ...
        for intpt = 1 : npoints
            ...
            if (ndof==2)
                if (elident(lmn)<2) % Plane stress or plane strain
                ...
                elseif (elident(lmn)==2)   % Axisymmetric
                    B = zeros(3,2*nelnodes(lmn));
                    B(1,1:2:2*nelnodes(lmn)-1) = dNdx(1:nelnodes(lmn),1);
                    B(2,2:2:2*nelnodes(lmn)) = dNdx(1:nelnodes(lmn),2);
                    B(3,1:2:2*nelnodes(lmn)-1) = N(1:nelnodes(lmn))/x(1);
                    B(4,1:2:2*nelnodes(lmn)-1) = dNdx(1:nelnodes(lmn),2);
                    B(4,2:2:2*nelnodes(lmn)) = dNdx(1:nelnodes(lmn),1);
                    D = E/(1+nu)/(1-2*nu)*[1-nu,nu,nu,0;...
                                          nu,1-nu,nu,0;...
                                          nu,nu,1-nu,0;...
                                          0,0,0,(1-2*nu)/2];
            ...
            if (ncoord == 2)
                if (elident(lmn)<2)
                   ...
                   fprintf(outfile,'%5d %7.4f %7.4f %9.4f %9.4f %9.4f %9.4f %9.4f %9.4f %9.4f %9.4f \n', ...
                    intpt,x(1),x(2),e(1),e(2),e(3),e(4)/2,s(1),s(2),s(3),s(4));       
                end
            elseif (ncoord == 3)
