...
%       mesh geometry and boundary conditions.  It uses the Newmark
%       time stepping method to integrate the discrete equations of
%       dynamic linear elasticity.
%
%       It can be run with the demonstration input file 
%       Linear_elastic_dynamic_beam.txt
%       It will produce an animation of the vibration of an 
%       initially straight cantilever beam that is loaded by a force
%       applied to its end.   Once the simulation is complete it will plot
%       a graph of the displacement of the end of the cantilever as a
%       function of time.   The mesh is coarse, so the predictions 
%       are not accurate, but can be used to explore the influence of
%       the various parameters in the Newmark algorithm, as well as
%       lumped or fully integrated mass matrices.
%
%
%
...
%
filename = '../Input Files/Linear_elastic_dynamic_beam.txt';
...
%
 ...
 %
     ...
     set(axes2,'FontName','Times','FontSize',16,...
         ...
         'Box','On');
     plot(vxy(1,:),vxy(2,:),'LineWidth',2,'Color','k');
     xlabel({'Time (AU)'},'FontName','Times','FontSize',16);
     ylabel({'End displacement (AU)'},'FontName','Times','FontSize',16);
