function FEM_selective_reduced_integration
...
%       modified to implement 'selective reduced integration' to avoid volumetric locking.
...
%filename = '../Input Files/volumetric_locking_demo_quadratic.txt';
    ...
    elident(:) = 4; % Set to 4 to activate selective reduced integration elements; 1 for fully integrated elements
...
%
   ...
   figure1 = figure('NumberTitle','off','Name','Figure','Color',[1 1 1]);
    ...
    a = coords(2,1);
     ...
     title({'Selective reduced integration demo', 'Blue - exact solution; red - FEA'})
...
%  Deviatoric contribution evaluated with full integration
   for intpt = 1 : npoints
      ...
      if (ndof==2)
        ...
        if (elident==4)  % Deviatoric D matrix for selective reduced integration
        ...
        D([1,8,15]) = (1-nu);
        D = E/(1+nu)/(1-2*nu)*D;
        if (elident==4)  % Selective reduced integration element
           D = eye(6)/2;
           D(1:3,1:3) = -1/3;
           D([1,8,15]) = 2/3;
...
%  Add the volumetric term if selective reduced integration is used
   if (elident==4)
      ...
      for intpt = 1 : npoints
          ...
          if (ndof==2)
            ...
            B(3,1:2:2*nelnodes-1) = dNdx(1:nelnodes,2);
            B(3,2:2:2*nelnodes) = dNdx(1:nelnodes,1);
            D = E/(3*(1-2*nu))*[1,1,0;1,1,0;0,0,0]; % Volumetric part of D
          else
            B = zeros(6,3*nelnodes);
            B(1,1:3:3*nelnodes-2) = dNdx(1:nelnodes,1);
            B(2,2:3:3*nelnodes-1) = dNdx(1:nelnodes,2);
            ...
            D = zeros(6);
            D(1:3,1:3) =1;
            D = E/(3*(1-2*nu))*D; % Volumetric part of D
          end           
