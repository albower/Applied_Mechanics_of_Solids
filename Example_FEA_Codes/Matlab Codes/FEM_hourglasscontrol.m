...
%       extended to implement 'hourglass control' in a finite element
%       simulation using reduced integration with linear
...
%                            For bricks and quads elident=4 means reduced
...
filename = '../Input Files/volumetric_locking_demo_linear.txt';
    ...
    elident(:) = 4;  % Set elident to 4 to activate hourglass control with reduced integration; 1 for fully integrated plane strain elements
    close all
%
...
%
    ...
    a = coords(2,1);
     ...
     title({'Hourglass Control Demo', 'Blue - exact solution; red - FEA'})
...
%
%
   if (elident<4)   % Standard element
     npoints = numberofintegrationpoints(ncoord,nelnodes,elident);
   else  % Reduced integration (with hourglass control for quad/brick elements)
...
%  Hourglass control (quads and bricks only)
   if (elident>3 && (nelnodes==4||nelnodes==8) )
      xi(1:ncoord) = 0;
      ...
      shearmodulus = materialprops(1)/(2*(1+materialprops(2)));
