function FEM_hourglassing_demo
%
%
%          Example 2D and 3D linear elastic FEM code 
%
%       The code is identical to the general purpose 2D/3D linear elastic
%       finite element code FEM_2Dor3D_linelast_standard.m, but is
%       set up to demonstrate 'houglassing' in a finite element
%       simulation using (un-modified) reduced integration with linear
...
%       exact solution.
%       The FEA solution looks scary - change the 'scale factor'
%       to reduce the displacement amplitude in the plot to see the
%       hourglass modes more clearly.
%
%       The code can also be used to demonstrate that reduced integration
%       will resolve locking in quadratic quadrilateral elements without
%       hourglassing.
%
...
% You can try the quadratic elements too - there will be no hourglassing
% and volumetric locking will be eliminated
...
%
    ...
    scalefactor = 1.;   % Change this to scale the displacements as needed
    for i = 1:nnode
        for j = 1:ndof
    ...
    a = coords(2,1);
     ...
     title({'Hourglassing Demo', 'Blue - exact solution; red - FEA'})
end
...
function n = numberofintegrationpoints(ncoord,nelnodes,elident)
 ...
 %  Lines below have been modified to use reduced integration
 %  for both linear and quadratic quadrilaterials
     if (nelnodes == 4)
         n = 1;
     end
     if (nelnodes == 8)
