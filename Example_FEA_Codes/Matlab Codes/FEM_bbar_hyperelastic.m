function FEM_bbar_hyperelastic

%
%          Example 2D and 3D hyperelastic FEM code
%          (with locking resistant B-bar elements)
%
%       The code reads an input file that defines material properties,
%       mesh geometry and boundary conditions.   It is set up
%       to run with input files Bbar-hypere_demo.txt or hypoelastic_quad4.txt
...
%
    filename = '../Input Files/Bbar_hypere_demo.txt'; %Use this file to demonstrate locking
%
...
%
    ...
    ylim([0 7]);
    hold on
    plot([4,5.2],[6.5,6.5],'LineWidth',2,'LineStyle','-','Color','k')
    plot([4,5.2],[5.75,5.75],'LineWidth',2,'LineStyle','--','Color','k')
    annotation(figure1,'textbox',...
        [0.685714285714286 0.837095239301523 0.144642853364348 0.0642857130794299],...
        'String',{'Deformed'},...
        'Interpreter','latex',...
        'FontSize',15,...
        ...
        'EdgeColor','none');
    annotation(figure1,'textbox',...
        [0.680357142857142 0.749000001206287 0.171428566851786 0.0642857130794299],...
        'String',{'Undeformed'},...
        'Interpreter','latex',...
        'FontSize',15,...
        'FontName','Times New Roman','EdgeColor','none');
        ponmu = num2str(1/materialprops(1));    
    annotation(figure1,'textbox',...
        [0.6875 0.658523810730095 0.10535714024944 0.0642857130794299],...
        'String',{['$p/\mu=',ponmu,'$']},...
        'Interpreter','latex',...
        'FontSize',15,...
        'FontName','Times New Roman','EdgeColor','none');
    ...
    % Create textbox
    konmu = num2str(materialprops(2)/materialprops(1));
    annotation(figure1,'textbox',...
        [0.692857142857142 0.563285715492001 0.146428567596844 0.0642857130794299],...
        'String',{['$K/\mu=',konmu,'$']},...
        'Interpreter','latex',...
        'FontSize',15,...
        'FontName','Times New Roman','EdgeColor','none');
...
%
   ...
   for intpt = 1 : npoints
      ...
      F = eye(3);
      F(1:ndof,1:ndof) = F(1:ndof,1:ndof) + displacement*dNdx;
%
      Finv = inv(F);
      J = det(F);
      
      dNdy(1:nelnodes,1:ncoord) = dNdx(1:nelnodes,1:ncoord)*Finv(1:ncoord,1:ncoord);

      dNbardy = dNbardy + J*dNdy*dt*w(intpt);
      Vel = Vel + dt*w(intpt);
      
      eta = eta + J*dt*w(intpt);
      
      if (ndof==2)
        for i = 1:nelnodes
          P(2*i-1,1:2:2*nelnodes-1) = dNdy(i,1);
          P(2*i-1,2:2:2*nelnodes) = dNdy(i,2);
          P(2*i,:) = P(2*i-1,:);
        end
      else
        for i = 1:nelnodes
          P(3*i-2,1:3:3*nelnodes-2) = dNdy(i,1);
          P(3*i-2,2:3:3*nelnodes-1) = dNdy(i,2);
          P(3*i-2,3:3:3*nelnodes) = dNdy(i,3);
          P(3*i-1,:) = P(3*i-2,:);
          P(3*i,:) = P(3*i-2,:);
        end
      
      end
      nu = reshape(transpose(dNdy),[1,nelnodes*ndof]);
      Q = Q + J*(tensorprod(nu,nu)-P.*transpose(P))*w(intpt)*dt;
      
   end
   
   eta = eta/Vel;
   dNbardy = dNbardy/(eta*Vel);
   Q = Q/(eta*Vel);
...
%
      J = det(F);
      Fbar = F*(eta/J)^(1/ndof);
      B = Fbar*transpose(Fbar);

      stress = Kirchhoffstress(ndof,ncoord,B,eta,materialprops);

      if (ndof==2)
          taujj = stress(1,1) + stress(2,2);
      ...
      D = materialstiffness(ndof,ncoord,B,eta,materialprops);
      Finv = inv(F);      
      dNdy(1:nelnodes,1:ncoord) = dNdx(1:nelnodes,1:ncoord)*Finv(1:ncoord,1:ncoord);      
      
      if (ndof==2)
        Abar = zeros(3,2*nelnodes);
        Abar(1,1:2:2*nelnodes-1) = dNdy(1:nelnodes,1);
        Abar(2,2:2:2*nelnodes) = dNdy(1:nelnodes,2);
        Abar(3,1:2:2*nelnodes-1) = dNdy(1:nelnodes,2);
        Abar(3,2:2:2*nelnodes) = dNdy(1:nelnodes,1);

         Abar(1,1:2:2*nelnodes-1) =  Abar(1,1:2:2*nelnodes-1) + transpose(dNbardy(1:nelnodes,1)-dNdy(1:nelnodes,1))/2;
         Abar(2,1:2:2*nelnodes-1) =  Abar(2,1:2:2*nelnodes-1) + transpose(dNbardy(1:nelnodes,1)-dNdy(1:nelnodes,1))/2;
         Abar(1,2:2:2*nelnodes)    = Abar(1,2:2:2*nelnodes)   + transpose(dNbardy(1:nelnodes,2)-dNdy(1:nelnodes,2))/2;
         Abar(2,2:2:2*nelnodes)    = Abar(2,2:2:2*nelnodes)   + transpose(dNbardy(1:nelnodes,2)-dNdy(1:nelnodes,2))/2;
         
         Abarstar = zeros(4,2*nelnodes);
         Abarstar(1:2,:) = Abar(1:2,:);
         Abarstar(3,1:2:2*nelnodes-1) = dNdy(1:nelnodes,2);
         Abarstar(4,2:2:2*nelnodes) = dNdy(1:nelnodes,1);
 
        G(1,1:4) = [B(1,1),0.d0,B(1,2),0.d0];
        G(2,1:4) = [0.d0,B(2,2),0.d0,B(1,2)];
        G(3,1:4) = [B(1,2),B(1,2),...
                                B(2,2),B(1,1)];
        G = 2*G;   
         
        taudNdy = dNdy*stress(1:2,1:2);
        
        for i = 1:nelnodes
          P(2*i-1,1:2:2*nelnodes-1) = dNdy(i,1);
          P(2*i-1,2:2:2*nelnodes) = dNdy(i,2);
          P(2*i,:) = P(2*i-1,:);
          S(1:2:2*nelnodes-1,2*i-1) = taudNdy(i,1);
          S(2:2:2*nelnodes,2*i-1) = taudNdy(i,2);
          S(:,2*i) = S(:,2*i-1);
        end
         
      else
        Abar = zeros(6,3*nelnodes);
        Abar(1,1:3:3*nelnodes-2) = dNdy(1:nelnodes,1);
        Abar(2,2:3:3*nelnodes-1) = dNdy(1:nelnodes,2);
        Abar(3,3:3:3*nelnodes)   = dNdy(1:nelnodes,3);
        Abar(4,1:3:3*nelnodes-2) = dNdy(1:nelnodes,2);
        Abar(4,2:3:3*nelnodes-1) = dNdy(1:nelnodes,1);
        Abar(5,1:3:3*nelnodes-2) = dNdy(1:nelnodes,3);
        Abar(5,3:3:3*nelnodes)   = dNdy(1:nelnodes,1);
        Abar(6,2:3:3*nelnodes-1) = dNdy(1:nelnodes,3);
        Abar(6,3:3:3*nelnodes)   = dNdy(1:nelnodes,2);

        Abar(1,1:3:3*nelnodes-2) = Abar(1,1:3:3*nelnodes-2) + transpose(dNbardy(1:nelnodes,1)-dNdy(1:nelnodes,1))/3;
        Abar(2,1:3:3*nelnodes-2) = Abar(2,1:3:3*nelnodes-2) + transpose(dNbardy(1:nelnodes,1)-dNdy(1:nelnodes,1))/3;
        Abar(3,1:3:3*nelnodes-2) = Abar(3,1:3:3*nelnodes-2) + transpose(dNbardy(1:nelnodes,1)-dNdy(1:nelnodes,1))/3;
        Abar(1,2:3:3*nelnodes-1) = Abar(1,2:3:3*nelnodes-1) + transpose(dNbardy(1:nelnodes,2)-dNdy(1:nelnodes,2))/3;
        Abar(2,2:3:3*nelnodes-1) = Abar(2,2:3:3*nelnodes-1) + transpose(dNbardy(1:nelnodes,2)-dNdy(1:nelnodes,2))/3;
        Abar(3,2:3:3*nelnodes-1) = Abar(3,2:3:3*nelnodes-1) + transpose(dNbardy(1:nelnodes,2)-dNdy(1:nelnodes,2))/3;
        Abar(1,3:3:3*nelnodes)   = Abar(1,3:3:3*nelnodes)   + transpose(dNbardy(1:nelnodes,3)-dNdy(1:nelnodes,3))/3;
        Abar(2,3:3:3*nelnodes)   = Abar(2,3:3:3*nelnodes)   + transpose(dNbardy(1:nelnodes,3)-dNdy(1:nelnodes,3))/3;        
        Abar(3,3:3:3*nelnodes)   = Abar(3,3:3:3*nelnodes)   + transpose(dNbardy(1:nelnodes,3)-dNdy(1:nelnodes,3))/3;        

        Abarstar = zeros(9,3*nelnodes);
        Abarstar(1:3,:) = Abar(1:3,:);
        Abarstar(4,1:3:3*nelnodes-2) = dNdy(1:nelnodes,2);
        Abarstar(5,2:3:3*nelnodes-1) = dNdy(1:nelnodes,1);
        Abarstar(6,1:3:3*nelnodes-2) = dNdy(1:nelnodes,3);
        Abarstar(7,3:3:3*nelnodes)   = dNdy(1:nelnodes,1);
        Abarstar(8,2:3:3*nelnodes-1) = dNdy(1:nelnodes,3);
        Abarstar(9,3:3:3*nelnodes) = dNdy(1:nelnodes,2);
        ...
        G(4,1:9) = [B(1,2),B(1,2),0.d0,...
                                B(2,2),B(1,1),B(2,3),0.d0,B(1,3), 0.d0];
        G(5,1:9) = [B(1,3),0.d0,B(1,3),...
                                B(2,3),0.d0,B(3,3),B(1,1),0.d0,B(1,2)];
        G(6,1:9) = [0.d0,B(2,3),B(2,3),...
                                 0.d0,B(1,3),0.d0,B(1,2),B(3,3),B(2,2)];
        ...
        for i = 1:nelnodes
          ...
          P(3*i-1,:) = P(3*i-2,:);
          P(3*i,:) = P(3*i-2,:);
          S(1:3:3*nelnodes-2,3*i-2) = taudNdy(i,1);
          S(2:3:3*nelnodes-1,3*i-2) = taudNdy(i,2);
          S(3:3:3*nelnodes,3*i-2) = taudNdy(i,3);
          S(:,3*i-1) = S(:,3*i-2);
          S(:,3*i) = S(:,3*i-2);
        end
      ...
      nubar = reshape(transpose(dNbardy),[1,nelnodes*ndof]);
      rel = rel + transpose(Abar)*stressvec*w(intpt)*dt;
      kel = kel + (transpose(Abar)*D*G*Abarstar - P.*S +...
             (taujj/ndof)*(Q - tensorprod(nubar,nubar) + P.*transpose(P)))*w(intpt)*dt;
   end
...
function stress = Kirchhoffstress(ndof,ncoord,B,J,materialprops)
   ...
   stress = mu1/J^(2/3)*(B-trace(B)/3*eye(3)) + K1*J*(J-1)*eye(3);

end

function D = materialstiffness(ndof,ncoord,B,J,materialprops)
   ...
   if (ndof==2)
      ...
      Binv = inv(B);
      ivec = [1,1,0,];
      bvec = [B(1,1),B(2,2),B(1,2)];
      bivec = [Binv(1,1),Binv(2,2),Binv(1,2)];
      D = D + (trace(B)/3*tensorprod(ivec,bivec) - tensorprod(ivec,ivec) - tensorprod(bvec,bivec))/3;
      D = D*mu1/J^(2/3);
      
      D = D + K1*J*(J-0.5)*tensorprod(ivec,bivec);
      ...
      Binv = inv(B);
      ivec = [1,1,1,0,0,0];
      bvec = [B(1,1),B(2,2),B(3,3),B(1,2),B(1,3),B(2,3)];
      bivec = [Binv(1,1),Binv(2,2),Binv(3,3),Binv(1,2),Binv(1,3),Binv(2,3)];
      D = D + (trace(B)/3*tensorprod(ivec,bivec) - tensorprod(ivec,ivec) - tensorprod(bvec,bivec))/3;
      D = D*mu1/J^(2/3);
...
function [resid,Stif] = globalmatrices(ncoord,ndof,nnode,coords,nelem,maxnodes,elident,nelnodes,connect,materialprops,dofs)
%
...
%
    ...
    n = nelnodes(lmn);
    ident = elident(lmn);
    [rel,kel] = elforcnstif(ncoord,ndof,n,ident,lmncoord,materialprops,lmndof);
%
...
%
    for lmn = 1:nelem
        ...
        w = integrationweights(ncoord,nelnodes(lmn),npoints,elident);

        for intpt = 1 : npoints

            xi(1:ncoord) = xilist(1:ncoord,intpt);

            dNdxi = shapefunctionderivs(nelnodes(lmn),ncoord,elident,xi);
            ...
            F = eye(3);
            F(1:ndof,1:ndof) = F(1:ndof,1:ndof) + displacements*dNdx;
            J = det(F);
        ...
        for intpt = 1 : npoints
            ...
            N = shapefunctions(nelnodes(lmn),ncoord,elident,xi);
            dNdxi = shapefunctionderivs(nelnodes(lmn),ncoord,elident,xi);
            x = (lmncoord+displacements)*N;
            dxdxi = lmncoord*dNdxi;
            dxidx = inv(dxdxi);
            ...
            stress = Kirchhoffstress(ndof,ncoord,B,eta,materialprops)/eta;
