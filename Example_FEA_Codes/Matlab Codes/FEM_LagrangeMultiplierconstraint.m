function FEM_LagrangeMultiplierconstraint
%
%          Example 2D and 3D linear elastic FEM code, extended to
%          enforce tie constraints between degrees of freedom for a
%          pair of nodes using Lagrange Multipliers.
%
...
%
%       It is set up to run with the input file Tie_constraint_example.txt
%       which sets up a simple simulation of two 8 noded brick elements
%       that are tied together with constraints, and loaded in uniaxial tension.
%       The simulation will plot the undeformed and deformed meshes side by side
%       You can check the output file to see that the constraint has been
%       imposed correctly.
%       
%
%
...
%                            (assumed uniform) 
%        nties               No. tie constraints
%        ties(i,c)           Node1, dof1, Node2, dof2 for tie number c
%
%
% ==================== Read data from the input file ===========================
   filename = '../Input Files/Tie_constraint_example.txt';
...
[nprops,materialprops,ncoord,ndof,nnode,coords,nelem,maxnodes,connect,nelnodes,elident,nfix,fixnodes,ndload,dloads,nties,ties] = read_input_file(infile);
fclose(infile);
...
%   Set up stiffness and force
    ...
    F = zeros(ndof*nnode+nties,1);
    lmults = zeros(nties,1);
    K = globalmatrices(ncoord,ndof,nnode,coords, ...
        nelem,maxnodes,elident,nelnodes,connect,materialprops,dofs,nties,ties,lmults);
...
%   Fix constrained nodes.
    ...
    %
    %        Solve for the displacement vector and Lagrange multipliers
    %
   sol = K\F;
...
set(axes1,'DataAspectRatio',[1 1 1],'FontName','Times','FontSize',16,...
    'LineWidth',1,'PlotBoxAspectRatio',[1 1 1.66666666666667],'XGrid','off',...
    'XLimitMethod','tight','YGrid','off','YLimitMethod','tight','ZLimitMethod',...
    'tight','Box','On');
 ...
 view(gca,[-18.0594202906859 9.54601297604127]); 
  
 title('Lagrange Multiplier constraint demo','FontSize',15)
...
%
function [kel,rel] = tiestiffness(ndof,dof1,dof2,dof1val,dof2val,lmultval)
...
%  It is included here for possible use in a nonlinear application  
   ...
   rel = zeros(2*ndof+1,1);
   rel(dof1) = lmultval;
   rel(ndof+dof2) = -lmultval;
   rel(2*ndof+1) = dof1val-dof2val;
   kel(dof1,2*ndof+1) = 1;
   kel(2*ndof+1,dof1) = 1;
   kel(ndof+dof2,2*ndof+1) = -1;
   kel(2*ndof+1,ndof+dof2) = -1;
end
...
function Stif = globalmatrices(ncoord,ndof,nnode,coords,nelem,maxnodes,elident,nelnodes,connect,materialprops,dofs,nties,ties,lmults)
%
%   Assemble the global stiffness matrix
%
   Stif = zeros(ndof*nnode+nties,ndof*nnode+nties);
...
%
   ...
   for c = 1:nties
       ...
       node1 = ties(1,c);
       node2 = ties(3,c);
       dof1val = dofs(ndof*(node1-1) + dof1);
       dof2val = dofs(ndof*(node2-1) + dof2);
       lmult = lmults(c);
       [ktie,~] = tiestiffness(ndof,dof1,dof2,dof1val,dof2val,lmult);
       for a = 1:2
           for i = 1:ndof
               rw = ndof*(ties(2*a-1,c)-1) + i;
               for b = 1:2
                   for j = 1:ndof
                       cl = ndof*(ties(2*b-1,c)-1) + j;
                       Stif(rw,cl) = Stif(rw,cl) + ktie(ndof*(a-1)+i,ndof*(b-1)+j);
                   end
               end
               cl = nnode*ndof+c;
               Stif(rw,cl) = Stif(rw,cl) + ktie(ndof*(a-1)+i,2*ndof+1);
               Stif(cl,rw) = Stif(cl,rw) + ktie(2*ndof+1,ndof*(a-1)+i);
           end
       end
   end
...
%      Print nodal displacements, element strains && stresses:a file
 ...
 function [nprops,materialprops,ncoord,ndof,nnode,coords,nelem,maxnodes, ...
                connect,nelnodes,elident,nfix,fixnodes,ndload,dloads,nties,ties] = read_input_file(infile) 
...
%
    ...
    cellno = cellno + 2;
    nties = str2num(cellarray{1}{cellno});
    ties = [];
    if (nties>0)
        cellno = cellno + 4;
        ties = zeros(2+ndof,ndload);
        for i = 1 : nties
            for j = 1 : 4
                cellno = cellno + 1;
                ties(j,i) = str2num(cellarray{1}{cellno});
            end
        end
    end
