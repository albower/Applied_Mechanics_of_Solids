...
%       extended to implement 'B-bar elements' to avoid volumetric locking.
...
%                                            full integration
%                                            elident=4 means plane strain Bbar element
...
%filename = '../Input Files/volumetric_locking_demo_quadratic.txt';
    ...
    elident(:) = 4; % Set to 4 to activate Bbar elements; 1 for fully integrated elements
...
%
   ...
   figure1 = figure('NumberTitle','off','Name','Figure','Color',[1 1 1]);
    ...
    a = coords(2,1);
     ...
     title({'B-bar element Demo', 'Blue - exact solution; red - FEA'})
...
%
   ...
   for intpt = 1 : npoints
      ...
      if (ndof==2)
        ...
        if (elident==4)  % Bbar element (plane strain only - plane stress does not lock)
