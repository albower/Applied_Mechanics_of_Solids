function fem_beam_Timoshenko
%
%          Example FEM code using elastic Timoshenko (shear flexible) beam elements
%
...
%       'FEM_beam_timoshenko_input.txt' it will plot a graph comparing the 
%       predicted deflection of a cantilever beam with the exact
%       (Euler-Bernoulli) solution.   With the data in the sample
%       input file, the additional deflection caused by shear flexibility
%       will be visible.   Changing the x-sect area of the beam in the
%       input file is a quick way to change the ratio of bending to
%       shear stiffness (larger A => larger shear stiffness)
%
...
%
filename = '../Input Files/FEM_beam_timoshenko_input.txt';
...
% Read the input file into a cell array
    ...
    [nbeams,n_total_nodes,first_node,coord,n_total_elements,first_element,connect,E,mu,e1dir,inertias,areas,betas,...
                            nfix,fixnodes,nload,loads] = read_file(cellarray) ;
    fclose(infile);

    plotexactsol = false;
    if (strcmp(filename,'../Input Files/FEM_beam_timoshenko_input.txt')) plotexactsol = true; end
...
%
    ...
    if (plotexactsol)
        ...
        annotation(figure1,'textbox',...
            [0.351785714285714 0.769047619047619 0.517857142857143 0.0870952380952403],...
            'String','Euler Beam Solution',...
            'Interpreter','latex',...
            'FontSize',15,...
        ...
        stiffact = round(E(1)*inertias(1,3)/(betas(1)*areas(1)*mu(1)*x(end)^2),2,'significant');
        annotation(figure1,'textbox',...
        [0.492857142857141 0.182333334752492 0.301785705823984 0.0714285700094133],...
        'String',{['$E I /(\beta \mu A L^2) = $',num2str(stiffact)]},...
            'Interpreter','latex',...
            'FontSize',15,...
            'FontName','Times New Roman',...
            'FitBoxToText','off',...
         ...
         title('Timoshenko Beam Demo','FontSize',15)
...
%  One point Gaussian integration points and weights        
        ...
        w(1) = 2.;
        xi(1) = 0.;
%        Uncomment to see the effects of 2 point integration        
%         n_int_pts = 2;        
%         w(1:2) = 1;
%         xi(1) = -1./sqrt(3);
%         xi(2) = 1./sqrt(3);
...
%
function [nbeams,n_total_nodes,first_node,coord,n_total_elements,first_element,connect,E,mu,e1dir,inertias,areas,betas,...
        nfix,fixnodes,nload,loads] = read_file(cellarray)
    %
