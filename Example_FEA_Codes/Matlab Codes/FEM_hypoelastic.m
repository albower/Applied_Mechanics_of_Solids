...
%       to run with input files hypoelastic_brick8.txt or hypoelastic_quad4.txt
%       which tests the code with a single 3D/2D (plane strain) element.
%       A graph will be plotted that shows the predicted stress-strain
%       curve for the material, obtained from the force-displacement
%       relation for the mesh.
%
...
%
    filename = '../Input Files/hypoelastic_quad4.txt';
%    filename = '../Input Files/hypoelastic_brick8.txt';
...
%
        ...
        fvu(1,step) = w(end)/coords(ndof,end);
    end
...
%
 ...
 %
     ...
     set(axes2,'FontName','Times','FontSize',16,...
         ...
         'Box','On');
     plot(fvu(1,:),fvu(2,:),'LineWidth',2,'Color','k');
     xlabel({'Strain'},'FontName','Times','FontSize',16);
     ylabel({'Stress'},'FontName','Times','FontSize',16);
     title('Demo of hypoelastic FEA code','FontName','Times','FontSize',16)
