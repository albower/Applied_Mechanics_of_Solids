function FEM_Cohesive_Zone
%
%          Example 2D and 3D linear elastic FEM code, extended to
%          include (nonlinear) small displacement cohesive zone elements
%          to model failure on an interface.
%
...
%       of each element and plots the displaced mesh.
%
%       It is set up to run with the input files CohesiveZone_example.txt
%       or CohesiveZone_example_2D.txt which set up a simple simulation of two
%       8 noded brick elements (or 4 noded plane stress elements)
%       that are bonded with a cohesive zone element, and loaded in uniaxial tension.
%       The simulation will plot a graph of the stress as a function of
%       displacement, and will display the deformed mesh at the end of
%       the simulation.
%       
...
%    the text.  Then change the fopen command below to point to the file.
...
%                            For 2D elements
%                               elident=1=>plane strain;
%                               elident=0=> plane stress
%                            elident=2 means cohesive zone element in this
%                            code
...
%
  filename = '../Input Files/CohesiveZone_example.txt';
%  filename = '../Input Files/CohesiveZone_example_2D.txt';
  ...
  forcevdisp = zeros(2,nsteps+1);
    ...
    for step = 1 : nsteps
        ...
        while ((err1>tol) && (nit<maxit))          % Newton Raphson loop
            ...
            [R,K] = globalmatrices(dt,ncoord,ndof,nnode,coords, ...
                nelem,maxnodes,elident,nelnodes,connect,materialprops,u,w);
...
%      Store traction and displacement for plotting later
    %
        u = u + w;
        s = (u(end)-u(ndof*(nelnodes(1)+1)))*materialprops(1)
        forcevdisp(1,step+1) = u(end)*materialprops(1)/(coords(end)*materialprops(3)); 
        forcevdisp(2,step+1) = s/materialprops(3) ; 
    ...
    set(axes2,'FontName','Times','FontSize',16,...
        ...
        'Box','On');
    plot(forcevdisp(1,:),forcevdisp(2,:),'LineWidth',2,'Color',[0,0,0]);
    ylabel('Stress $\sigma/\sigma_{max}$','FontName','Times','Interpreter','latex');
    xlabel('Displacement $E u/2L\sigma_{max}$','FontName','Times','Interpreter','latex');
    hold on
    Lam = materialprops(1)*materialprops(4)/(coords(end)*materialprops(3))';
    dd = [0:0.05:10];
    ss = dd.*exp(1-dd);
    uu = dd.*(Lam+exp(1-dd));
    plot(uu,ss,'LineStyle','--','LineWidth',2,'Color',[0,0,0])
    xlim([0 2.5])
    ylim([0 1.25])
    % Create textarrow
    annotation(figure1,'textarrow',[0.576785714285714 0.476785714285714],...
        [0.797619047619048 0.716666666666667],...
        'String',['Finite Element',sprintf('\n'),'(rate dependent)'],...
        'Interpreter','latex',...
        'FontSize',15,...
        'FontName','Times New Roman');
    
    % Create textarrow
    annotation(figure1,'textarrow',[0.601785714285714 0.430357142857143],...
        [0.728571428571429 0.602380952380952],'String',['Exact solution',sprintf('\n'),'(rate indep.)'],...
        'Interpreter','latex',...
        'FontSize',15,...
        'FontName','Times New Roman');

    title('Cohesive Zone Demo')
...
function [r,kel] = elforcnstif_cz(dt,ncoord,ndof,nnode,elident,coord,materialprops,displacement,displacement_increment)
...
%
    ...
    Delta = B*displacement(:);
    dDelta = B*displacement_increment(:);
    D = interface_stiffness(dt,materialprops,Delta,dDelta);    
    Tract = interface_traction(dt,materialprops,Delta,dDelta);
...
function T = interface_traction(dt,materialprops,Delta0,dDelta)

    Delta = Delta0 + dDelta;
    smax = materialprops(3);
    dn = materialprops(4);
    eta = materialprops(5);
    ...
    if (length(Delta)==2)
        Deltat = abs(Delta(2));
        c1 = exp(1-Delta(1)/dn)*exp(-Deltat^2/dn^2);
        Tn = smax*Delta(1)/dn*c1 + eta*dDelta(1)/dt;
        T1 = 2*smax*Delta(2)/dn*(1+Delta(1)/dn)*c1 + eta*dDelta(2)/dt;
        T = [Tn;T1];
 
    else
        Deltat = norm(Delta(2:3));
        c1 = exp(1-Delta(1)/dn)*exp(-Deltat^2/dn^2);
        Tn = smax*Delta(1)/dn*c1 + eta*dDelta(1)/dt;
        T1 = 2*smax*Delta(2)/dn*(1+Delta(1)/dn)*c1 + eta*dDelta(2)/dt;
        T2 = 2*smax*Delta(3)/dn*(1+Delta(1)/dn)*c1 + eta*dDelta(2)/dt;
        T = [Tn;T1;T2];
    end
...
function D = interface_stiffness(dt,materialprops,Delta0,dDelta)
    ...
    if (length(Delta)==2)
        Deltat = abs(Delta(2));
        c1 = exp(1-Delta(1)/dn)*exp(-Deltat^2/dn^2);
        dTnddn = smax/dn*(1-Delta(1)/dn)*c1 + eta/dt;
        dTndd1 = -2*(smax/dn)*(Delta(1)*Delta(2)/dn^2)*c1;
        dT1ddn = dTndd1;
        dT1dd1 = 2*smax/dn*(1-2*Delta(2)^2/dn^2)*(1+Delta(1)/dn)*c1 + eta/dt;
        D = [dTnddn,dTndd1;...
            dT1ddn, dT1dd1];
        ...
        dTnddn = smax/dn*(1-Delta(1)/dn)*c1 + eta/dt;
        dTndd1 = -2*(smax/dn)*(Delta(1)*Delta(2)/dn^2)*c1;
        dTndd2 = -2*(smax/dn)*(Delta(1)*Delta(3)/dn^2)*c1;
        dT1ddn = dTndd1;
        dT2ddn = dTndd2;
        dT1dd1 = 2*smax/dn*(1-2*Delta(2)^2/dn^2)*(1+Delta(1)/dn)*c1 + eta/dt;
        dT2dd2 = 2*smax/dn*(1-2*Delta(3)^2/dn^2)*(1+Delta(1)/dn)*c1 + eta/dt;
        dT1dd2 = -4*smax/dn*(Delta(2)*Delta(3)/dn^2)*(1+Delta(1)/dn)*c1;
        dT2dd1 = dT1dd2;

        D = [dTnddn, dTndd1, dTndd2;...
             dT1ddn, dT1dd1, dT1dd2;...
             dT2ddn, dT2dd1, dT2dd2];
    end
...
function [resid,Stif] = globalmatrices(dt,ncoord,ndof,nnode,coords,nelem,maxnodes,elident,nelnodes,connect,materialprops,dofs,ddofs)
%
...
%
    ...
    if (ident<2)
       ...
       [rel,kel] = elforcnstif_cz(dt,ncoord,ndof,n,ident,lmncoord,materialprops,lmndof,dlmndof);
    end
            %
