function FEM_2Dor3D_modeshapes
%
%          Example 2D and 3D dynamic linear elastic FEM code 
%
%       The code reads an input file that defines material properties,
%       mesh geometry and boundary conditions.  It calculates the mode
%       shapes and frequencies for the discrete equations of motion.
%
%       It can be run with the demonstration input file 
%       Linear_elastic_dynamic_beam.txt
%       It will print the full set of frequencies to the command window
%       and will plot the mode shapes for the four lowest (nonzero)
%       frequency modes.
%       The mesh in the demo file is very coarse, so the results
...
for i=1:nfix
    rw=ndof*(fixnodes(1,i)-1)+fixnodes(2,i);
    for j=1:ndof*nnode
        K(rw,j)=0;
        M(rw,j) = 0;
        M(j,rw) = 0;
...
%
 %  Set up matrices to do modal decomposition
 %
 rootM = sqrtm(M);
...
%      Lambda has eigenvalues in decreasing order. The last nz
...
% No. zero frequency modes is 3 (2D), 6(3D), or # constrained DOFs
 ...
 for mode = 1:4 
   ...
   u = inverserootM*Q(:,ndof*nnode-nzfmodes-mode+1);
 
   scale = 3.;
   for i = 1 : nnode
     for j = 1 : ndof
       defcoords(j,i) = coords(j,i) + scale*u(ndof*(i-1)+j);
     end
   end
  figure
  plotmesh(defcoords,ncoord,nnode,connect,nelem,elident,nelnodes,'r')
  titstr = ['Mode ' int2str(mode)];
