function FEM_dynamic_constraints
%
%          Example 2D and 3D dynamic linear elastic FEM code, extended to
%          enforce tie constraints between degrees of freedom for a
%          pair of nodes using Lagrange Multipliers.  The equations of
%          motion are integrated using the explicit Newmark algorithm with a 
%          predictor-corrector step to enforce the constraints
%
%       The code reads an input file that defines material properties,
%       mesh geometry and boundary conditions.  
%
%       It is set up to run with the input file dynamic_constrained_beam.txt
%       which sets up a simple simulation of a vibrating cantilever beam,
%       with two elements connected using tie constraints.   The code will
%       animate the vibration.  There is a gap between the tied elements
%       so the constrained motion of the connected nodes is visible.
%       
%
...
%    editor window)
%
%          Example explicit dynamic code with Lagrange Multipliers enforcing tie constraints 
%
...
%                            (assumed uniform) 
%        nties               No. tie constraints enforced by Lagrange Multiplier method
%        ties(i,j)           List of nodes in tie constraints
%                            ties(1,j) Node number
%                            ties(2,j) DOF number for node 1
%                            ties(3,j) Node number
%                            ties(4,j) DOF number for node 2
%
%  To run the program you first need to set up an input file, as described in 
%  the text.  Then change the fopen command below to point to the file.
%  Also change the fopen command in the post-processing step (near the bottom of the
...
%
%
filename = '../Input Files/dynamic_constrained_beam.txt';
...
%  Fix constrained nodes. 
  ...
  Mdiag = diag(M);   % Extract only the diagonal of the lumped mass matrix.
...
%
 ...
 %  Predictor step   
    ...
    % Corrector step   
       ...
       an1 = an1bar;
       for c = 1:nties
          rw = ndof*(ties(1,c)-1) + ties(2,c);
          Kcons(1,1) = Mdiag(rw);
          Kcons(1,3) = 1;
          Kcons(3,1) = 1;
          Rcons(1) = Mdiag(rw)*an1bar(rw);
          Rcons(3) = -( un1(rw)+dt*(vn(rw)+0.5*dt*an(rw)) )/dt^2;
          rw = ndof*(ties(3,c)-1) + ties(4,c);
          Kcons(2,2) = Mdiag(rw);
          Kcons(2,3) = -1;
          Kcons(3,2) = -1;
          Rcons(2) = Mdiag(rw)*an1bar(rw);
          Rcons(3) = Rcons(3) +( un1(rw)+dt*(vn(rw)+0.5*dt*an(rw)) )/dt^2;          
       ...
       vn1 = (vn + 0.5*dt*an + 0.5*dt*an1);
       un1 = (un + dt*vn1 + 0.5*dt*dt*an1);
