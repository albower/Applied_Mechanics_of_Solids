...
%       modified to implement 'hybrid elements' to avoid volumetric locking.
%       It calculates and plots the displacement field in 
%       a pressurized cylinder, and compares the finite element solution to the
%       exact solution.   Hybrid elements will work with fully
%       incompressible materials, so you can test this code with Poisson's
%       ratio set to 0.5
%
%
...
%
filename = '../Input Files/volumetric_locking_demo_linear.txt';
%filename = '../Input Files/volumetric_locking_demo_quadratic.txt';
    ...
    elident(:) = 4; % Set to 4 to activate hybrid elements; 1 for fully integrated elements
...
%   Set up stiffness and force
    ...
    F = globaltraction(ncoord,ndof,nnode,nelem,ndload,coords, ...
...
%
   ...
   figure1 = figure('NumberTitle','off','Name','Figure','Color',[1 1 1]);
    ...
    a = coords(2,1);
     ...
     title({'Hybrid element demo', 'Blue - exact solution; red - FEA'})
...
%
   ...
   if (elident==4) % No. pressure points on a hybrid element
      npress = numberofpressurepoints(ncoord,nelnodes,elident);
   else
       npress = 0;
   end
...
%  Deviatoric contribution evaluated with full integration
   for intpt = 1 : npoints
      ...
      if (npress>1) 
          M = shapefunctions(npress,ncoord,elident,xi);
      else
          M(1) = 1.0;
      end
      dxdxi = coord*dNdxi;
      ...
      if (ndof==2)
        ...
        if (elident>0) % Regular plane strain D matrix
           D = E/(1+nu)/(1-2*nu)*[1-nu,nu,0;nu,1-nu,0;0,0,(1-2*nu)/2];
        end
        if (elident==4)  % Hybrid element uses deviatoric part of D
           D = E/(1+nu)*[2/3,-1/3,0;-1/3,2/3,0;0,0,1/2];
        end
      else
        ...
        D([1,8,15]) = (1-nu);
        D = E/(1+nu)/(1-2*nu)*D;
        if (elident==4)  % Hybrid element
           D = eye(6)/2;
           D(1:3,1:3) = -1/3;
           D([1,8,15]) = 2/3;
           D = E/(1+nu)*D;   % Deviatoric part of D
        end
      end           

      kel(1:ndof*nelnodes,1:ndof*nelnodes) = kel(1:ndof*nelnodes,1:ndof*nelnodes) + (transpose(B)*D*B)*w(intpt)*dt;
      if (elident==4)  % Add the pressure terms to the element stiffness matrix for hybrid element
          kel(ndof*nelnodes+1:end,ndof*nelnodes+1:end) = kel(ndof*nelnodes+1:end,ndof*nelnodes+1:end) - 3*(1-2*nu)*tensorprod(M,M)*w(intpt)*dt/E;
          for a = 1:ndof*nelnodes
             kel(a,ndof*nelnodes+1:end) =  kel(a,ndof*nelnodes+1:end) + (B(1,a)+B(2,a))*M(1:end)'*w(intpt)*dt;
             kel(ndof*nelnodes+1:end,a) = kel(ndof*nelnodes+1:end,a) + (B(1,a)+B(2,a))*M(1:end)*w(intpt)*dt;
             if (ndof==3)
                kel(a,ndof*nelnodes+1:end) =  kel(a,ndof*nelnodes+1:end) + B(3,a)*M(1:end)'*w(intpt)*dt;
                kel(ndof*nelnodes+1:end,a) = kel(ndof*nelnodes+1:end,a) + B(3,a)*M(1:end)*w(intpt)*dt;
             end
          end
      end
   end
...
% Can comment this out for recent MATLAB versions
% it is now a built-in function.
...
%
   npress = 0;
   if (elident==4)
   npress = numberofpressurepoints(ncoord,nelnodes(1),elident(1));
   end
...
function r = globaltraction(ncoord,ndof,nnodes,nelem,ndload,coords,nelnodes,elident,connect,dloads)
...
%   each element type
%
function n = numberofpressurepoints(ncoord,nelnodes,elident)
